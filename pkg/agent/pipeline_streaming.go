...
func (p *Pipeline) tryConfiguredStreamingLLM(
	...
	agentName := strings.TrimSpace(ts.agent.Name)
	if agentName == "" {
		agentName = strings.TrimSpace(ts.agent.ID)
	}
...
func (p *streamingChunkPublisher) Update(ctx context.Context, accumulated string) {
	...
	if setter, ok := p.streamer.(interface{ SetAgentName(agentName string) }); ok {
		setter.SetAgentName(p.agentName)
	}
